...
st.markdown("Kurlar **TCMB** üzerinden canlı çekilmektedir. Kazanan teklifler tablo içerisinde **yeşil** ile vurgulanmaktadır.")
...
if uploaded_file:
    ...
    if not tedarikci_sutunlari:
        ...
        sonuc_df = df.copy()
        en_ucuz_tedarikciler, en_ucuz_fiyatlar, fiyat_avantajlari = [], [], []
            ...
            for col_name in tedarikci_sutunlari:
                ...
                if deger is not None and str(deger).strip() != "":
                    ...
                    if sayi is not None:
                        ...
                        firma_bilgileri[tl_deger] = (col_name, f"{sayi} {birim}")

            if teklifler_tl:
                teklifler_tl.sort()
                en_ucuz_tl = teklifler_tl[0]
                en_ucuz_tedarikciler.append(firma_bilgileri[en_ucuz_tl][0])
                en_ucuz_fiyatlar.append(firma_bilgileri[en_ucuz_tl][1])
                
                if len(teklifler_tl) > 1:
                    fiyat_avantajlari.append(round(teklifler_tl[1] - teklifler_tl[0], 2))
                else:
                    fiyat_avantajlari.append(0.00)
            else:
        ...
        sonuc_df['Fiyat Avantajı (TL)'] = fiyat_avantajlari
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            winner_cell_fmt = workbook.add_format({'bg_color': '#A9D08E', 'border': 1}) # Fiyat sütunundaki kazanan hücre (Yeşil)
            summary_fmt = workbook.add_format({'bold': True, 'fg_color': '#FFEB9C', 'border': 1}) # Özet sütunları (Sarı)
            advantage_fmt = workbook.add_format({'bold': True, 'fg_color': '#C6EFCE', 'font_color': '#006100', 'border': 1}) # Avantaj (Açık Yeşil)
            border_fmt = workbook.add_format({'border': 1})

            last_col = len(sonuc_df.columns) - 1
            row_count = len(sonuc_df)

            # Başlıkları yaz ve sütun genişliğini ayarla
            for col_num, value in enumerate(sonuc_df.columns.values):
                worksheet.write(0, col_num, value, header_fmt)
                worksheet.set_column(col_num, col_num, 18)

            # Verileri satır satır tara ve kazanan hücreyi boya
            for row_idx in range(row_count):
                kazanan_firma = sonuc_df.iloc[row_idx]['En Uygun Tedarikçi']
                
                # Tedarikçi sütunlarında gez
                for col_idx, col_name in enumerate(tedarikci_sutunlari):
                    excel_col_idx = 6 + col_idx # İlk 6 sütun ürün bilgisi
                    hucre_degeri = sonuc_df.iloc[row_idx][col_name]
                    
                    if col_name == kazanan_firma:
                        worksheet.write(row_idx + 1, excel_col_idx, hucre_degeri, winner_cell_fmt)
                    else:
                        worksheet.write(row_idx + 1, excel_col_idx, hucre_degeri, border_fmt)

            # Özet sütunlarını boya
            worksheet.conditional_format(1, last_col - 2, row_count, last_col - 1, {'type': 'no_errors', 'format': summary_fmt})
            worksheet.conditional_format(1, last_col, row_count, last_col, {'type': 'no_errors', 'format': advantage_fmt})
