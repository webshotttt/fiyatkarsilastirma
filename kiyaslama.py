...
st.markdown("Kurlar küresel piyasalardan otomatik çekilmektedir. Analiz edilecek veri setini yükleyiniz.")

# Arka planda 60 saniyede bir güncellenen canlı kur motoru
...
def canli_kurlari_cek():
    try:
        ...
        usd_kur = usd_data['Close'].iloc[-1]
        eur_kur = eur_data['Close'].iloc[-1]
        
        # Senin istediğin tarih-saat formatı
...
if canli_usd and canli_eur:
    st.success(f"✅ Kurlar Canlı Çekildi. (Güncellenme Tarihi: {son_guncelleme})")
...
# Kur girişleri (Otomatik dolar/euro ile dolar, istenirse elle değiştirilebilir)
...
if uploaded_file:
    # 1. Pandas ile iskeleti oku
    uploaded_file.seek(0)
    df = pd.read_excel(uploaded_file)
    
    # 2. Openpyxl ile arka plandaki hücre formatlarını oku
    uploaded_file.seek(0)
            ...
            for col_name in tedarikci_sutunlari:
                ...
                if sayi is not None and str(sayi).strip() != "":
                    ...
                    if sayi_float is not None:
                        # 2. Para Birimini Çekme (Excel bug önleyici)
