...
st.markdown("Kurlar **Google Finans** üzerinden anlık çekilmektedir. Analiz raporuna **Dinamik Kıyaslamalı Analiz** sütunu eklenmiştir.")

# Google Finans Canlı Kur Motoru (60 Saniyede Bir Güncellenir)
@st.cache_data(ttl=60)
def canli_kurlari_cek():
    try:
        # Tarayıcı taklidi yapıyoruz ki Google bizi bot sanıp engellemesin
        headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36'}
        
        # USD/TRY Çekimi
        req_usd = urllib.request.Request("https://www.google.com/finance/quote/USD-TRY", headers=headers)
        with urllib.request.urlopen(req_usd) as response:
            html_usd = response.read().decode('utf-8')
        # Google'ın arka plan kodundan son fiyatı cımbızlıyoruz
        match_usd = re.search(r'data-last-price="([^"]+)"', html_usd)
        usd_kur = float(match_usd.group(1)) if match_usd else None
        
        # EUR/TRY Çekimi
        req_eur = urllib.request.Request("https://www.google.com/finance/quote/EUR-TRY", headers=headers)
        with urllib.request.urlopen(req_eur) as response:
            html_eur = response.read().decode('utf-8')
        match_eur = re.search(r'data-last-price="([^"]+)"', html_eur)
        eur_kur = float(match_eur.group(1)) if match_eur else None

        if usd_kur and eur_kur:
            # Saniyesine kadar saati alıyoruz
...
if canli_usd and canli_eur:
    st.success(f"✅ Kurlar Google'dan Anlık Alındı. (Son Güncelleme: {son_guncelleme})")
    default_usd, default_eur = float(canli_usd), float(canli_eur)
else:
    st.warning("⚠️ Google canlı verisine ulaşılamadı, lütfen kurları manuel giriniz.")
...
if uploaded_file:
    ...
    if not tedarikci_sutunlari:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            winner_cell_fmt = workbook.add_format({'bg_color': '#A9D08E', 'border': 1})
            summary_fmt = workbook.add_format({'bold': True, 'fg_color': '#FFEB9C', 'border': 1})
            analysis_fmt = workbook.add_format({'bold': True, 'fg_color': '#C6EFCE', 'font_color': '#006100', 'border': 1})
